  ...
  >
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      {services.map(s => {
        ...
        return (
          <div key={s.id} className="bg-white border border-slate-200 rounded-2xl p-7 hover:border-[#FF2A2A]/40 hover:shadow-lg transition-all relative overflow-hidden group">
            <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-[#FF2A2A]/5 to-transparent rounded-bl-full" />
            <div className="relative flex gap-5">
              <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#FF2A2A] to-[#00C2E6] flex items-center justify-center flex-shrink-0 shadow-md group-hover:scale-105 transition-transform">
                <Icon className="w-7 h-7 text-white" strokeWidth={2} />
              </div>
              <div className="flex-1">
                <h3 className="font-bold text-[#003D52] text-xl mb-2">{s.title}</h3>
                ...
                <ul className="space-y-1.5 mb-5">
                  {['Expert document review', 'Real-time tracking', '24/7 support'].map(b => (
                    <li key={b} className="flex items-center gap-2 text-xs text-slate-600">
                      <Check className="w-3.5 h-3.5 text-[#FF2A2A]" strokeWidth={3} />
                      {b}
                    </li>
                  ))}
                </ul>
                <Link to="/apply" className="text-sm font-semibold text-[#FF2A2A] inline-flex items-center gap-1 hover:gap-2 transition-all">
    ...
    <div className="mt-16 text-center bg-gradient-to-br from-[#003D52] to-[#005C75] rounded-3xl p-12">
