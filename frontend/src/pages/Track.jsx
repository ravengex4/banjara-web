      ...
      <div className="max-w-2xl mx-auto">
        <form onSubmit={handleTrack} className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
          ...
          <div className="flex gap-3">
            ...
            <Button type="submit" className="bg-[#FF2A2A] hover:bg-[#E01F1F] text-white h-12 px-6 gap-2">
          ...
          <div className="mt-8 bg-white border border-slate-200 rounded-2xl p-7 shadow-sm">
            <div className="flex items-center justify-between flex-wrap gap-3 mb-6 pb-5 border-b border-slate-100">
              <div>
                ...
                <div className="text-xl font-bold text-[#003D52]">{result.ref}</div>
              </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-7">
              ...
              <div><div className="text-xs text-slate-500 mb-1">Visa Type</div><div className="font-semibold text-[#003D52] text-sm">{result.visaType}</div></div>
              <div><div className="text-xs text-slate-500 mb-1">Submitted</div><div className="font-semibold text-[#003D52] text-sm">{result.submitted}</div></div>
              <div><div className="text-xs text-slate-500 mb-1">Expected</div><div className="font-semibold text-[#003D52] text-sm">{result.expected}</div></div>
            </div>
              ...
              {result.timeline.map((t, i) => (
                <div key={i} className="flex gap-4">
                  <div className="flex flex-col items-center">
                    ...
                    {i < result.timeline.length - 1 && <div className={`w-0.5 flex-1 ${t.completed ? 'bg-[#FF2A2A]' : 'bg-slate-200'}`} />}
                  </div>
